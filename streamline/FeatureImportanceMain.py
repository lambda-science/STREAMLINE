...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="do_mutual_info",
        type=str,
        help="do mutual information analysis",
        default="True",
    )
    parser.add_argument(
        "--do-ms",
    ...
    parser.add_argument(
        ...
        dest="use_TURF",
        type=str,
        help="use TURF wrapper around MultiSURF to improve feature interaction detection in large feature spaces (only recommended if you have reason to believe at least half of your features are non-informative)",
        default="False",
    )
    parser.add_argument(
        "--turf-pct",
    ...
    parser.add_argument(
        ...
        dest="n_jobs",
        type=int,
        help="number of cores dedicated to running algorithm; setting to -1 will use all available cores",
        default=1,
    )
    parser.add_argument(
        "--inst-sub",
        dest="instance_subset",
        type=int,
        help="sample subset size to use with multiSURF",
        default=2000,
    )
    ...
    if not os.path.exists(options.output_path):
        raise Exception(
            "Output path must exist (from phase 1) before phase 3 can begin"
        )
    ...
    if not options.do_check:  # Run job file
        ...
        for dataset_directory_path in dataset_paths:
            ...
            if eval(options.do_mutual_info):
                if not os.path.exists(
                    full_path + "/feature_selection/mutualinformation"
                ):
                    os.mkdir(full_path + "/feature_selection/mutualinformation")
                for cv_train_path in glob.glob(
                    full_path + "/CVDatasets/*_CV_*Train.csv"
                ):
                    command_text = (
                        ...
                        + instance_label
                        + " "
                        + str(options.instance_subset)
                        + " mi "
                        + str(options.n_jobs)
                        + " "
                        + str(options.use_TURF)
                        + " "
                        + str(options.TURF_pct)
                    )
                    job_counter += 1
        ...
        for dataset in datasets:
            for cv in range(cv_partitions):
                if eval(options.do_multisurf):
                    phase3Jobs.append(
                        "job_multisurf_" + dataset + "_" + str(cv) + ".txt"
                    )
                if eval(options.do_mutual_info):
                    phase3Jobs.append(
                        "job_mutualinformation_" + dataset + "_" + str(cv) + ".txt"
                    )
...
def submitClusterJob(
    command_text, experiment_path, reserved_memory, maximum_memory, queue, jupyterRun
):
