...
def job(
    ...
    data_train, data_test, header, dataset_name, cvCount = loadData(
        cv_train_path, cv_test_path, experiment_path, class_label, instance_label
    )
    if eval(jupyterRun):
        print(
            "Preparing Train and Test for: " + str(dataset_name) + "_CV_" + str(cvCount)
        )
    ...
    if eval(scale_data):
        ...
        x_train, x_test = dataScaling(
            x_train, x_test, experiment_path, dataset_name, cvCount
        )
    ...
    if instance_label == None or instance_label == "None":
        data_train = pd.concat(
            [
                pd.DataFrame(y_train, columns=[class_label]),
        ...
        data_train = pd.concat(
            [
                pd.DataFrame(y_train, columns=[class_label]),
                pd.DataFrame(i_train, columns=[instance_label]),
                pd.DataFrame(x_train, columns=header),
            ],
            axis=1,
            sort=False,
    ...
    if instance_label == None or instance_label == "None":
        data_test = pd.concat(
            [
                pd.DataFrame(y_test, columns=[class_label]),
        ...
        data_test = pd.concat(
            [
                pd.DataFrame(y_test, columns=[class_label]),
                pd.DataFrame(i_test, columns=[instance_label]),
                pd.DataFrame(x_test, columns=header),
            ],
            axis=1,
            sort=False,
    ...
    job_file = open(
        experiment_path
        + "/jobsCompleted/job_preprocessing_"
        + dataset_name
        + "_"
        + str(cvCount)
        + ".txt",
...
def imputeCVData(
    ...
    outfile = open(
        ...
        + "/"
        + dataset_name
        + "/scale_impute/categorical_imputer_cv"
        + str(cvCount)
        + ".pickle",
        "wb",
    )
    ...
    if eval(multi_impute):
        ...
        outfile = open(
            ...
            + "/"
            + dataset_name
            + "/scale_impute/ordinal_imputer_cv"
            + str(cvCount)
            + ".pickle",
            "wb",
        )
...
def dataScaling(x_train, x_test, experiment_path, dataset_name, cvCount):
    ...
    x_train = pd.DataFrame(
        scaler.transform(x_train).round(decimal_places), columns=x_train.columns
    )
    ...
    x_test = pd.DataFrame(
        scaler.transform(x_test).round(decimal_places), columns=x_test.columns
    )
...
def saveRuntime(experiment_path, dataset_name, job_start_time):
    ...
    runtime_file = open(
        experiment_path + "/" + dataset_name + "/runtime/runtime_preprocessing.txt", "w"
    )
