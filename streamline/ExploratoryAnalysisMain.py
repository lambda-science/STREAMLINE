...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="class_label",
        type=str,
        help="outcome label of all datasets",
        default="Class",
    )
    parser.add_argument(
        "--inst-label",
    ...
    parser.add_argument(
        "--fi",
        dest="ignore_features_path",
        type=str,
        help="path to .csv file with feature labels to be ignored in analysis (e.g. /home/ryanurb/code/STREAMLINE/droppedFeatures.csv))",
        default="None",
    )
    parser.add_argument(
        "--cf",
        dest="categorical_feature_path",
        type=str,
        help="path to .csv file with feature labels specified to be treated as categorical where possible",
        default="None",
    )
    ...
    parser.add_argument(
        ...
        dest="cv_partitions",
        type=int,
        help="number of CV partitions",
        default=10,
    )
    parser.add_argument(
        "--part",
        dest="partition_method",
        type=str,
        help="'S', or 'R', or 'M', for stratified, random, or matched, respectively",
        default="S",
    )
    parser.add_argument(
        "--match-label",
        dest="match_label",
        type=str,
        help="only applies when M selected for partition-method; indicates column with matched instance ids",
        default="None",
    )
    parser.add_argument(
        "--cat-cutoff",
        dest="categorical_cutoff",
        type=int,
        help="number of unique values after which a variable is considered to be quantitative vs categorical",
        default=10,
    )
    parser.add_argument(
        "--sig",
        dest="sig_cutoff",
        type=float,
        help="significance cutoff used throughout pipeline",
        default=0.05,
    )
    parser.add_argument(
        "--export-fc",
        dest="export_feature_correlations",
        type=str,
    ...
    if not options.do_check:  # Run job file
        makeDirTree(
            options.data_path, options.output_path, options.experiment_name, jupyterRun
        )  # check file/path names and create directory tree for output
        ...
        for dataset_path in glob.glob(options.data_path + "/*"):
            ...
            if file_extension == "txt" or file_extension == "csv":
                if data_name not in unique_datanames:
                    ...
                    ):  # Run as job in parallel on linux computing cluster
                        submitClusterJob(
                            dataset_path,
                            options.output_path + "/" + options.experiment_name,
                            options.cv_partitions,
                            options.partition_method,
                            options.categorical_cutoff,
                            options.export_feature_correlations,
                            options.export_univariate_plots,
                            options.class_label,
                            ...
                            options.queue,
                            options.ignore_features_path,
                            options.categorical_feature_path,
                            options.sig_cutoff,
                            jupyterRun,
                        )
...
def makeDirTree(data_path, output_path, experiment_name, jupyterRun):
    ...
    if os.path.exists(output_path + "/" + experiment_name):
        raise Exception(
            ...
            + "/"
            + experiment_name
            + ". This path/folder name must be unique."
        )
...
def submitClusterJob(
    ...
    sh_file.write(
        ...
        + str(random_state)
        + " "
        + str(ignore_features_path)
        + " "
        + str(categorical_feature_path)
        + " "
        + str(sig_cutoff)
        + " "
        + str(jupyterRun)
