...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="metric_weight",
        type=str,
        help="ML model metric used as weight in composite FI plots (only supports balanced_accuracy or roc_auc as options) Recommend setting the same as primary_metric if possible.",
        default="balanced_accuracy",
    )
    parser.add_argument(
        "--top-features",
        dest="top_model_features",
        type=int,
    ...
    if not os.path.exists(options.output_path):
        raise Exception(
            "Output path must exist (from phase 1) before phase 6 can begin"
        )
    ...
    primary_metric = metadata[
        "Primary Metric"
    ]  # 0 list position is the True/False for algorithm use
    ...
    if not options.do_check:  # Run job submission
        ...
        for dataset_directory_path in dataset_paths:
            ...
            if eval(do_DT) and not os.path.exists(
                full_path + "/model_evaluation/DT_Viz"
            ):
                os.mkdir(full_path + "/model_evaluation/DT_Viz")
            if eval(do_GP) and not os.path.exists(
                full_path + "/model_evaluation/GP_Viz"
            ):
            ...
            if eval(options.run_parallel):
                submitClusterJob(
                    ...
                    primary_metric,
                    options.top_model_features,
                    sig_cutoff,
                    options.metric_weight,
                    jupyterRun,
                )
            else:
                submitLocalJob(
        ...
        df.to_csv(
            options.output_path + "/" + options.experiment_name + "/" + "metadata.csv",
            index=True,
        )
...
def submitClusterJob(
    ...
    sh_file.write(
        ...
        + scale_data
        + " "
        + str(plot_metric_boxplots)
        + " "
        + str(primary_metric)
        + " "
        + str(top_model_features)
        + " "
        + str(sig_cutoff)
        + " "
        + str(metric_weight)
