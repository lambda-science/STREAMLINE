...
def job(
    ...
    dataset_name, dataFeatures, dataOutcome, header, cvCount = prepareData(
        cv_train_path, instance_label, class_label
    )
    if eval(jupyterRun):
        print(
            "Prepared Train and Test for: " + str(dataset_name) + "_CV_" + str(cvCount)
        )
    # Apply mutual information if specified by user
    if algorithm == "mi":
        if eval(jupyterRun):
            print("Running Mutual Information...")
        scores, outpath, alg_name = runMutualInformation(
            experiment_path,
            dataset_name,
            cvCount,
            dataFeatures,
    ...
    # Apply MultiSURF if specified by user
        ...
        scores, outpath, alg_name = runMultiSURF(
            dataFeatures,
            dataOutcome,
            instance_subset,
            experiment_path,
    ...
    scoreDict, score_sorted_features = sort_save_fi_scores(
        scores, header, outpath, alg_name
    )
...
def runMutualInformation(
    experiment_path, dataset_name, cvCount, dataFeatures, dataOutcome, random_state
):
...
def runMultiSURF(
    ...
    choices = np.random.choice(
        formatted.shape[0], min(instance_subset, formatted.shape[0]), replace=False
    )
    newL = []
    for i in choices:
