...
def job(
    ...
    result_table, metric_dict = primaryStats(
        algorithms,
        original_headers,
        cv_partitions,
        full_path,
    ...
    if len(algorithms) > 1:
        ...
        kruskal_summary = kruskalWallis(
            full_path, metrics, algorithms, metric_dict, sig_cutoff
        )
        wilcoxonRank(
            full_path, metrics, algorithms, metric_dict, kruskal_summary, sig_cutoff
        )
        mannWhitneyU(
            full_path, metrics, algorithms, metric_dict, kruskal_summary, sig_cutoff
        )
    ...
    (
        ...
        algorithms, full_path, abbrev, metric_dict, metric_weight
    )  # old - 'Balanced Accuracy'
    # Select 'top' features for composite vizualization
    featuresToViz = selectForCompositeViz(
        top_model_features,
        non_zero_union_features,
        non_zero_union_indexes,
        algorithms,
    ...
    top_fi_med_norm_list, all_feature_listToViz = getFI_To_Viz_Sorted(
        featuresToViz, all_feature_list, algorithms, fi_med_norm_list
    )  # If we want composite FI plots to be displayed in descenting total bar height order.
    # Generate Normalized composite FI plot
    composite_FI_plot(
        top_fi_med_norm_list,
        algorithms,
        list(colors.values()),
    ...
    composite_FI_plot(
        ...
        full_path,
        jupyterRun,
        "Normalized and Weighted Median Feature Importance",
    )
    ...
    job_file = open(
        experiment_path + "/jobsCompleted/job_stats_" + data_name + ".txt", "w"
    )
...
def preparation(full_path, algInfo):
    ...
    original_headers = pd.read_csv(
        full_path + "/exploratory/OriginalFeatureNames.csv", sep=","
    ).columns.values.tolist()  # Get Original Headers
...
def primaryStats(
    ...
    for algorithm in algorithms:  # completed for each individual ML modeling algorithm
        ...
        FI_all = (
            []
        )  # used to save model feature importances individually for each cv within single summary file (all original features in dataset prior to feature selection included)
        ...
        mean_recall = np.linspace(
            0, 1, 100
        )  # used to plot average of CV line in PRC plot
        ...
        aveprecs = (
            []
        )  # stores individual CV average precisions for PRC to calculate CV average
        ...
        for cvCount in range(0, cv_partitions):
            ...
            s_ck.append(metricList[13])
            s_mcc.append(metricList[14])
            s_ll.append(metricList[15])
            s_f1w.append(metricList[16])
            s_f1m.append(metricList[17])
            # update list that stores values used in ROC and PRC plots
            alg_result_table.append(
        ...
        if eval(plot_ROC):
            ...
            plt.savefig(
                full_path + "/model_evaluation/" + abbrev[algorithm] + "_ROC.png",
                bbox_inches="tight",
            )
            if eval(jupyterRun):
                plt.show()
        ...
        if eval(plot_PRC):
            ...
            test = pd.read_csv(
                full_path + "/CVDatasets/" + data_name + "_CV_0_Test.csv"
            )  # Technically there could be a unique no-skill line for each CV dataset based on final class balance (however only one is needed, and stratified CV attempts to keep partitions with similar/same class balance)
            ...
            plt.plot(
                ...
                mean_prec,
                color=colors[algorithm],
                label=r"Mean PRC (AUC = %0.3f $\pm$ %0.3f)" % (mean_pr_auc, std_pr_auc),
                lw=2,
                alpha=0.8,
            )
            # Plot standard deviation grey zone of curves
            ...
            # Export and/or show plot
            plt.savefig(
                full_path + "/model_evaluation/" + abbrev[algorithm] + "_PRC.png",
                bbox_inches="tight",
            )
            if eval(jupyterRun):
                plt.show()
        ...
        filepath = (
            full_path + "/model_evaluation/" + abbrev[algorithm] + "_performance.csv"
        )
...
def save_FI(FI_all, algorithm, globalFeatureList, full_path):
    ...
    filepath = (
        full_path + "/model_evaluation/feature_importance/" + algorithm + "_FI.csv"
    )
...
def doPlotPRC(
    result_table, colors, full_path, data_name, instance_label, class_label, jupyterRun
):
    ...
    for i in result_table.index:
        plt.plot(
            ...
            label="{}, AUC={:.3f}, APS={:.3f}".format(
                i, result_table.loc[i]["pr_auc"], result_table.loc[i]["ave_prec"]
            ),
        )
        count += 1
    # Estimate no skill line based on the fraction of cases found in the first test dataset
...
def saveMetricMedians(full_path, metrics, metric_dict):
    ...
    with open(
        full_path + "/model_evaluation/Summary_performance_median.csv",
        mode="w",
        newline="",
    ) as file:
        writer = csv.writer(
...
def saveMetricMeans(full_path, metrics, metric_dict):
    ...
    with open(
        full_path + "/model_evaluation/Summary_performance_mean.csv",
        mode="w",
        newline="",
    ) as file:
        writer = csv.writer(
...
def saveMetricStd(full_path, metrics, metric_dict):
    ...
    with open(
        full_path + "/model_evaluation/Summary_performance_std.csv",
        mode="w",
        newline="",
    ) as file:
        writer = csv.writer(
...
def metricBoxplots(full_path, metrics, algorithms, metric_dict, jupyterRun):
    ...
    for metric in metrics:
        ...
        # Export and/or show plot
        plt.savefig(
            full_path + "/model_evaluation/metricBoxplots/Compare_" + metric + ".png",
            bbox_inches="tight",
        )
        if eval(jupyterRun):
            plt.show()
...
def kruskalWallis(full_path, metrics, algorithms, metric_dict, sig_cutoff):
    ...
    kruskal_summary.to_csv(
        full_path + "/model_evaluation/statistical_comparisons/KruskalWallis.csv"
    )
        ...
        if kruskal_summary["Sig(*)"][metric] == "*":
            ...
            for algorithm1 in algorithms:
                for algorithm2 in algorithms:
                    if (
                        not [algorithm1, algorithm2] in done
                        and not [algorithm2, algorithm1] in done
                        and algorithm1 != algorithm2
                    ):
            ...
            wilcoxon_stats_df.to_csv(
                full_path
                + "/model_evaluation/statistical_comparisons/WilcoxonRank_"
                + metric
                + ".csv",
                index=False,
            )
...
def mannWhitneyU(
    ...
    for metric in metrics:
        if kruskal_summary["Sig(*)"][metric] == "*":
            ...
            mann_stats_df.to_csv(
                full_path
                + "/model_evaluation/statistical_comparisons/MannWhitneyU_"
                + metric
                + ".csv",
                index=False,
            )
...
def prepFI(algorithms, full_path, abbrev, metric_dict, metric_weight):
    ...
    for algorithm in algorithms:
        ...
        temp_df = pd.read_csv(
            ...
            + abbrev[algorithm]
            + "_FI.csv"
        )  # CV FI scores for all original features in dataset.
        if (
        ...
        fi_med_list.append(
            temp_df.median().tolist()
        )  # Saves median FI scores over CV runs
    ...
    for each in fi_med_list:  # each algorithm
        ...
        for i in range(len(each)):  # each feature (score) in original data order
            if (
                each[i] <= 0
            ):  # Feature importance scores assumed to be uninformative if at or below 0
                normList.append(0)
            else:
    ...
    alg_non_zero_FI_list = (
        []
    )  # stores list of feature name lists that are non-zero for each algorithm
    for each in fi_med_list:  # each algorithm
        temp_non_zero_list = []
        for i in range(len(each)):  # each feature
            if each[i] > 0.0:
                temp_non_zero_list.append(
                    all_feature_list[i]
...
def selectForCompositeViz(
    ...
    for each in non_zero_union_features:  # for each non-zero feature
        for j in range(len(algorithms)):  # for each algorithm
            ...
            if (
                weight <= 0.5
            ):  # This is why this method is limited to balanced_accuracy and roc_auc
                weight = 0
            if not weight == 0:
    ...
    scoreSumDict_features = sorted(
        scoreSumDict, key=lambda x: scoreSumDict[x], reverse=True
    )
    if (
        len(non_zero_union_features) > top_model_features
...
def doFIBoxplots(
    ...
    for algorithm in algorithms:  # each algorithms
        ...
        plt.xlabel("Features")
        plt.xticks(
            np.arange(1, len(featuresToViz) + 1), featuresToViz, rotation="vertical"
        )
        plt.savefig(
            full_path
            + "/model_evaluation/feature_importance/"
        ...
        if eval(jupyterRun):
            ...
            plt.close(
                "all"
            )  # Identify and sort (decreaseing) features with top median FI
        algorithmCounter += 1
...
def getFI_To_Viz_Sorted(featuresToViz, all_feature_list, algorithms, fi_med_norm_list):
    ...
    top_fi_med_norm_list = (
        []
    )  # feature importance values of top features for each algorithm (list of lists)
...
def composite_FI_plot(
    ...
    bottoms = (
        []
    )  # list of space used by previous algorithms for each feature (so next bar can be placed directly above it)
    for i in range(len(algorithms) - 1):
        for j in range(i + 1):
            if j == 0:
                bottom = np.array(fi_list[0])
            else:
                np.add(bottom, np.array(fi_list[j]), out=bottom, casting="unsafe")
    ...
    plt.xticks(
        np.arange(len(all_feature_listToViz)),
        all_feature_listToViz,
        rotation="vertical",
    )
    ...
    plt.savefig(
        full_path
        + "/model_evaluation/feature_importance/Compare_FI_"
        + figName
        + ".png",
        bbox_inches="tight",
    )
...
def fracFI(top_fi_med_norm_list):
    ...
    for each in top_fi_med_norm_list:  # each algorithm
        ...
        for i in range(len(each)):  # each feature
            if (
                sum(each) == 0
            ):  # check that all feature scores are not zero to avoid zero division error
                fracList.append(0)
            else:
