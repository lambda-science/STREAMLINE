...
def job(experiment_path, training, rep_data_path, data_path):
    ...
    analy_report.cell(
        w=180,
        h=8,
        txt="STREAMLINE Training Summary Report: " + time,
        ln=2,
        border=1,
        align="L",
    )
    ...
    analy_report.multi_cell(
        w=90, h=4, txt="General Pipeline Settings:", border=1, align="L"
    )
    ...
    analy_report.multi_cell(
        w=90,
        h=4,
        txt=" " + listToString(ls1) + " " + listToString(ls3) + " " + listToString(ls5),
        border=1,
        align="L",
    )
    ...
    analy_report.set_font("Times", "B", 10)
    analy_report.multi_cell(
        w=90, h=4, txt="ML Modeling Algorithms:", border=1, align="L"
    )
    analy_report.y += 1  # Space below section header
    analy_report.set_font("Times", "", 8)
    analy_report.x += 90
    ...
    analy_report.set_font("Times", "B", 10)
    analy_report.multi_cell(
        w=90, h=4, txt="LCS Settings (eLCS,XCS,ExSTraCS):", border=1, align="L"
    )
    analy_report.y += 1  # Space below section header
    analy_report.set_font("Times", "", 8)
    analy_report.x += 90
    ...
    if tryAgain:
        try:  # Running on Google Colab
            analy_report.image(
                "/content/drive/MyDrive/STREAMLINE/Pictures/STREAMLINE_LOGO.png",
                102,
                150,
                90,
            )
    ...
    if eval(training):
        ...
        analy_report.cell(
            w=180,
            h=4,
            txt="Target Training Dataset: " + train_name,
            border=1,
            align="L",
        )
        analy_report.y += 5
    ...
    if eval(training):
        ...
        for page in range(0, pageCount):  # generate each page
            pubUnivariate(
                analy_report, experiment_path, ds, page, resultLimit, pageCount
            )
    ...
    for n in range(len(ds)):
        ...
        analy_report.cell(
            w=0,
            h=8,
            txt="Dataset and Model Prediction Summary:  D" + str(n + 1) + " = " + ds[n],
            border=1,
            align="L",
            ln=2,
        )
        ...
        if eval(training):
            analy_report.image(
                experiment_path + "/" + ds[n] + "/exploratory/ClassCountsBarPlot.png",
                1,
                10,
                60,
                40,
        ...
        if eval(training):
            data_summary = pd.read_csv(
                experiment_path + "/" + ds[n] + "/exploratory/DataCounts.csv"
            )
        else:
            data_summary = pd.read_csv(
                experiment_path
        ...
        analy_report.multi_cell(
            w=60, h=4, txt="Dataset Counts Summary:", border=1, align="L"
        )
        analy_report.set_font("Times", "", 8)
        analy_report.multi_cell(
            w=60, h=4, txt=" " + listToString(info_ls), border=1, align="L"
        )
        ...
        algorithm = summary_performance[
            summary_performance["ROC AUC"] == highest_ROC
        ].index.values
        best_alg_ROC = summary_performance.iloc[algorithm, 0]

        summary_performance["Bal Acc"] = summary_performance["Bal Acc"].astype(float)
        highest_BA = summary_performance["Bal Acc"].max()
        algorithm = summary_performance[
            summary_performance["Bal Acc"] == highest_BA
        ].index.values
        best_alg_BA = summary_performance.iloc[algorithm, 0]

        summary_performance["F1"] = summary_performance["F1"].astype(float)
        highest_F1 = summary_performance["F1"].max()
        algorithm = summary_performance[
            summary_performance["F1"] == highest_F1
        ].index.values
        ...
        algorithm = summary_performance[
            summary_performance["PRC AUC"] == highest_PRC
        ].index.values
        ...
        algorithm = summary_performance[
            summary_performance["PRC APS"] == highest_APS
        ].index.values
        ...
        analy_report.multi_cell(
            w=80,
            h=4,
            txt="Top ML Algorithm Results (Averaged Over CV Runs):",
            border=1,
            align="L",
        )
        analy_report.set_font("Times", "", 8)
        ...
        if len(best_alg_BA.values) > 1:
            analy_report.multi_cell(
                w=80,
                h=4,
                txt="Best (Balanced Acc.): "
                + str(best_alg_BA.values[0])
                + " (TIE) = "
        ...
        if len(best_alg_APS.values) > 1:
            analy_report.multi_cell(
                ...
                h=4,
                txt="Best (PRC APS): "
                + str(best_alg_APS.values[0])
                + " (TIE) = "
                + str("{:.3f}".format(highest_APS)),
                border=1,
                align="L",
            )
        else:
        ...
        if eval(training):
            analy_report.image(
                experiment_path + "/" + ds[n] + "/model_evaluation/Summary_ROC.png",
                4,
                118,
                120,
            )
        ...
        if eval(training):
            analy_report.image(
                experiment_path + "/" + ds[n] + "/model_evaluation/Summary_PRC.png",
                4,
                206,
                133,
    ...
    if eval(training):
        ...
        for k in range(len(ds)):
            ...
            analy_report.cell(
                w=0,
                h=8,
                txt="Feature Importance Summary:  D" + str(k + 1) + " = " + ds[k],
                border=1,
                align="L",
                ln=2,
            )
            ...
            analy_report.image(
                ...
                + "/"
                + ds[k]
                + "/feature_selection/mutualinformation/TopAverageScores.png",
                5,
                12,
                100,
                135,
            )  # Images adjusted to fit a width of 100 and length of 135
            analy_report.image(
                experiment_path
                + "/"
                + ds[k]
                + "/feature_selection/multisurf/TopAverageScores.png",
                105,
                12,
                100,
                135,
            ...
            analy_report.image(
                ...
                + "/"
                + ds[k]
                + "/model_evaluation/feature_importance/Compare_FI_Norm_Weight.png",
                1,
                159,
                208,
                125,
    ...
    if eval(training):
        ...
        analy_report.cell(
            w=0,
            h=8,
            txt="Compare ML Performance Across Datasets",
            border=1,
            align="L",
            ln=2,
        )
        ...
        if len(ds) > 1:
            analy_report.image(
                experiment_path
                + "/DatasetComparisons/dataCompBoxplots/"
        ...
        if success:
            ...
            if len(ds) <= 3:  # 4
                ...
                for row in kw_ds:
                    for datum in row:
                        analy_report.cell(
                            col_width_list[col_count], th, str(datum), border=1
                        )
                        col_count += 1
                    col_count = 0
                ...
                if len(ds) > 12:
                    ...
                    analy_report.cell(
                        0,
                        4,
                        "Warning: Additional dataset results could not be displayed",
                        1,
                        align="C",
                    )
    ...
    try:
        if eval(training):
            ...
            try:
                job_file = open(
                    experiment_path + "/jobsCompleted/job_data_pdf_training.txt", "w"
                )
            ...
            try:
                job_file = open(
                    experiment_path
                    + "/jobsCompleted/job_data_pdf_apply_"
                    + str(train_name)
                    + ".txt",
                    "w",
                )
...
def pubUnivariate(analy_report, experiment_path, ds, page, resultLimit, pageCount):
    ...
    if pageCount > 1:
        analy_report.cell(
            w=180,
            h=8,
            txt="Univariate Analysis of Each Dataset (Top 10 Features for Each): Page "
            + str(page + 1),
            border=1,
            align="L",
            ln=2,
        ...
        analy_report.cell(
            w=180,
            h=8,
            txt="Univariate Analysis of Each Dataset (Top 10 Features for Each)",
            border=1,
            align="L",
            ln=2,
        )
    try:  # Try loop added to deal with versions specific change to using mannwhitneyu in scipy and avoid STREAMLINE crash in those circumstances.
        for n in range(dataStart, datasetCount):
            ...
            analy_report.set_font("Times", "B", 10)
            analy_report.multi_cell(
                w=180, h=4, txt="D" + str(n + 1) + " = " + ds[n], border=1, align="L"
            )
            analy_report.y += 1  # Space below section header
            analy_report.set_font("Times", "B", 8)
            analy_report.multi_cell(
                w=180, h=4, txt="Feature:  P-Value", border=1, align="L"
            )
            analy_report.set_font("Times", "", 8)
            analy_report.multi_cell(
                w=180, h=4, txt=" " + listToString(sig_ls), border=1, align="L"
            )
    except:
        analy_report.x = 5
        analy_report.y = 40
        analy_report.cell(
            180,
            4,
            "WARNING: Univariate analysis failed from scipy package error. To fix: pip install --upgrade scipy",
            1,
            align="L",
        )
    footer(analy_report)
...
def pubModelMeanStats(
    ...
    if pageCount > 1:
        ...
        analy_report.cell(
            w=0,
            h=8,
            txt="Average Model Prediction Statistics (Rounded to 3 Decimal Points)",
            border=1,
            align="L",
            ln=2,
        )
...
def pubRuntime(analy_report, experiment_path, ds, page, resultLimit, pageCount):
    ...
    if pageCount > 1:
        analy_report.cell(
            w=0,
            h=8,
            txt="Pipeline Runtime Summary: Page " + str(page + 1),
            border=1,
            align="L",
            ln=2,
        )
        ...
        analy_report.cell(
            w=0, h=8, txt="Pipeline Runtime Summary", border=1, align="L", ln=2
        )
    analy_report.set_font("Times", "", 8)
    th = analy_report.font_size
    analy_report.y += 2
...
def footer(self):
    ...
    self.cell(
        0,
        7,
        "Generated with the URBS-Lab STREAMLINE: (https://github.com/UrbsLab/STREAMLINE)",
        0,
        0,
        "C",
    )
