...
def main(argv):
    ...
    # No defaults
    parser.add_argument(
        "--out-path", dest="output_path", type=str, help="path to output directory"
    )
    parser.add_argument(
        "--exp-name",
        dest="experiment_name",
        type=str,
        help="name of experiment (no spaces)",
    )
    ...
    parser.add_argument(
        ...
        dest="do_all",
        type=str,
        help="run all modeling algorithms by default (when set False, individual algorithms are activated individually)",
        default="True",
    )
    ...
    parser.add_argument(
        ...
        dest="do_LR",
        type=str,
        help="run logistic regression modeling",
        default="None",
    )
    parser.add_argument(
        "--do-DT",
        dest="do_DT",
        type=str,
        help="run decision tree modeling",
        default="None",
    )
    parser.add_argument(
        "--do-RF",
        dest="do_RF",
        type=str,
        help="run random forest modeling",
        default="None",
    )
    parser.add_argument(
        "--do-GB",
        dest="do_GB",
        type=str,
        help="run gradient boosting modeling",
        default="None",
    )
    parser.add_argument(
        "--do-XGB", dest="do_XGB", type=str, help="run XGBoost modeling", default="None"
    )
    parser.add_argument(
        "--do-LGB", dest="do_LGB", type=str, help="run LGBoost modeling", default="None"
    )
    parser.add_argument(
        "--do-CGB",
        dest="do_CGB",
        type=str,
        help="run CatBoost modeling",
        default="None",
    )
    parser.add_argument(
        "--do-SVM",
        dest="do_SVM",
        type=str,
        help="run support vector machine modeling",
        default="None",
    )
    parser.add_argument(
        "--do-ANN",
        dest="do_ANN",
        type=str,
        help="run artificial neural network modeling",
        default="None",
    )
    parser.add_argument(
        "--do-KNN",
        dest="do_KNN",
        type=str,
        help="run k-nearest neighbors classifier modeling",
        default="None",
    )
    parser.add_argument(
        "--do-GP",
        dest="do_GP",
        type=str,
        help="run genetic programming symbolic classifier modeling",
        default="None",
    )
    ...
    parser.add_argument(
        ...
        dest="do_XCS",
        type=str,
        help="run XCS modeling (a supervised-learning-only implementation of the best studied learning classifier system)",
        default="False",
    )
    parser.add_argument(
        "--do-ExSTraCS",
    ...
    parser.add_argument(
        ...
        dest="primary_metric",
        type=str,
        help="primary scikit-learn specified scoring metric used for hyperparameter optimization and permutation-based model feature importance evaluation",
        default="balanced_accuracy",
    )
    parser.add_argument(
        "--subsample",
        dest="training_subsample",
        type=int,
        help="for long running algos (XGB,SVM,ANN,KN), option to subsample training set (0 for no subsample)",
        default=0,
    )
    parser.add_argument(
        "--use-uniformFI",
        dest="use_uniform_FI",
        type=str,
        help="overrides use of any available feature importance estimate methods from models, instead using permutation_importance uniformly",
        default="True",
    )
    ...
    parser.add_argument(
        ...
        dest="n_trials",
        type=str,
        help="# of bayesian hyperparameter optimization trials using optuna (specify an integer or None)",
        default=200,
    )
    parser.add_argument(
        "--timeout",
        dest="timeout",
        type=str,
        help="seconds until hyperparameter sweep stops running new trials (Note: it may run longer to finish last trial started) If set to None, STREAMLINE is completely replicable, but will take longer to run",
        default=900,
    )  # 900 sec = 15 minutes default
    parser.add_argument(
        "--export-hyper-sweep",
        dest="export_hyper_sweep_plots",
        type=str,
    ...
    parser.add_argument(
        ...
        dest="do_lcs_sweep",
        type=str,
        help="do LCS hyperparam tuning or use below params",
        default="False",
    )
    parser.add_argument(
        "--nu",
        dest="nu",
        type=int,
        help="fixed LCS nu param (recommended range 1-10), set to larger value for data with less or no noise",
        default=1,
    )
    parser.add_argument(
        "--iter",
        dest="iterations",
        type=int,
        help="fixed LCS # learning iterations param",
        default=200000,
    )
    parser.add_argument(
        "--N",
        dest="N",
        type=int,
        help="fixed LCS rule population maximum size param",
        default=2000,
    )
    parser.add_argument(
        "--lcs-timeout",
        dest="lcs_timeout",
        type=int,
        help="seconds until hyperparameter sweep stops for LCS algorithms",
        default=1200,
    )
    ...
    parser.add_argument(
        ...
        dest="run_parallel",
        type=str,
        help="if run parallel on LSF compatible computing cluster",
        default="True",
    )
    parser.add_argument(
        "--queue",
        dest="queue",
        type=str,
        help="specify name of parallel computing queue (uses our research groups queue by default)",
        default="i2c2_normal",
    )
    parser.add_argument(
        "--res-mem",
    ...
    parser.add_argument(
        ...
        dest="maximum_memory",
        type=int,
        help="maximum memory before the job is automatically terminated",
        default=15,
    )
    parser.add_argument(
        "-c",
        "--do-check",
        dest="do_check",
        help="Boolean: Specify whether to check for existence of all output files.",
        action="store_true",
    )
    parser.add_argument(
        "-r",
        "--do-resubmit",
        dest="do_resubmit",
        help="Boolean: Rerun any jobs that did not complete (or failed) in an earlier run.",
        action="store_true",
    )
    ...
    if not os.path.exists(options.output_path):
        raise Exception(
            "Output path must exist (from phase 1) before phase 5 can begin"
        )
    ...
    file = open(
        options.output_path + "/" + options.experiment_name + "/" + "metadata.pickle",
        "rb",
    )
    ...
    if (
        options.do_resubmit
    ):  # Attempts to resolve optuna hyperparameter optimization hangup (i.e. when it runs indefinitely for a given random seed attempt)
    ...
    pickle_out = open(
        options.output_path + "/" + options.experiment_name + "/" + "algInfo.pickle",
        "wb",
    )
    ...
    if not options.do_check and not options.do_resubmit:  # Run job submission
        ...
        removeList = [
            ...
            "jobs",
            "DatasetComparisons",
            options.experiment_name + "_ML_Pipeline_Report.pdf",
        ]
                ...
                for algorithm in algorithms:
                    ...
                    if eval(options.run_parallel):
                        submitClusterJob(
                            ...
                            options.timeout,
                            options.lcs_timeout,
                            options.export_hyper_sweep_plots,
                            instance_label,
                            class_label,
                            random_state,
                            ...
                            options.do_lcs_sweep,
                            options.nu,
                            options.iterations,
                            options.N,
                            options.training_subsample,
                            options.queue,
                            options.use_uniform_FI,
        ...
        metadata["Extreme Gradient Boosting"] = str(
            algInfo["Extreme Gradient Boosting"][0]
        )
        metadata["Light Gradient Boosting"] = str(algInfo["Light Gradient Boosting"][0])
        metadata["Category Gradient Boosting"] = str(
            algInfo["Category Gradient Boosting"][0]
        )
        metadata["Support Vector Machine"] = str(algInfo["Support Vector Machine"][0])
        metadata["Artificial Neural Network"] = str(
            algInfo["Artificial Neural Network"][0]
        )
        ...
        metadata[
            "Training Subsample for KNN,ANN,SVM,and XGB"
        ] = options.training_subsample
        metadata[
            "Uniform Feature Importance Estimation (Models)"
        ] = options.use_uniform_FI
            ...
            for cv in range(cv_partitions):
                for algorithm in algorithms:
                    phase5Jobs.append(
                        ...
                        + algInfo[algorithm][1]
                        + ".txt"
                    )  # use algorithm abreviation for filenames

        for filename in glob.glob(
            options.output_path
            + "/"
            + options.experiment_name
            + "/jobsCompleted/job_model*"
        ):
        ...
        options.do_resubmit and not options.do_check
    ):  # resubmit any jobs that didn't finish in previous run (mix of job check and job submit)
        datasets = os.listdir(options.output_path + "/" + options.experiment_name)
        removeList = removeList = [
            "metadata.pickle",
            "metadata.csv",
        ...
        print(
            "Run options in conflict. Do not request to run check and resubmit at the same time."
        )
