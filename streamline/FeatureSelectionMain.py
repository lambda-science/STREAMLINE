...
def main(argv):
    ...
    if not os.path.exists(options.output_path):
        raise Exception(
            "Output path must exist (from phase 1) before phase 4 can begin"
        )
    ...
    if not options.do_check:  # Run job file
        ...
        for dataset_directory_path in dataset_paths:
            ...
            if eval(options.run_parallel):
                submitClusterJob(
                    ...
                    do_mutual_info,
                    do_multisurf,
                    options.max_features_to_keep,
                    options.filter_poor_features,
                    options.top_features,
                    options.export_scores,
                    class_label,
                    instance_label,
                    cv_partitions,
        ...
        for filename in glob.glob(
            ...
            + "/"
            + options.experiment_name
            + "/jobsCompleted/job_featureselection*"
        ):
