...
def job(
    ...
    feature_names = pd.read_csv(
        full_path + "/CVDatasets/" + data_name + "_CV_" + str(cvCount) + "_Test.csv"
...
def runModel(
    ...
    trainX, trainY, testX, testY = dataPrep(
        train_file_path, instance_label, class_label, test_file_path
    )
...
def saveRuntime(full_path, job_start_time, algAbrev, algorithm, cvCount):
    ...
    runtime_file = open(
        full_path + "/runtime/runtime_" + algAbrev + "_CV" + str(cvCount) + ".txt", "w"
    )
...
def hyper_eval(est, x_train, y_train, random_state, hype_cv, params, scoring_metric):
    ...
    performance = np.mean(
        cross_val_score(
            model, x_train, y_train, cv=cv, scoring=scoring_metric, verbose=0
        )
    )
    return performance
...
def modelEvaluation(clf, model, x_test, y_test):
    ...
    metricList = classEval(y_test, y_pred, probas_)
    ...
    fpr, tpr, roc_auc = scoring_curve.score_roc_curve(y_test, probas_, classes)
    prec, recall, prec_rec_auc, ave_prec = scoring_curve.score_precision_recall(
...
def run_NB_full(
    ...
    # Save model
    pickle.dump(
        model, open(full_path + "/models/pickledModels/NB_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
# Logistic Regression ###################################################################################################################
def objective_LR(
    trial, est, x_train, y_train, random_state, hype_cv, param_grid, scoring_metric
):
    ...
    params = {
        ...
        "max_iter": trial.suggest_loguniform(
            "max_iter", param_grid["max_iter"][0], param_grid["max_iter"][1]
        ),
...
def run_LR_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                ...
                fig.write_image(
                    full_path + "/models/LR_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for LR Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        ...
        export_best_params(
            full_path + "/models/LR_bestparams" + str(i) + ".csv", best_trial.params
        ...
        export_best_params(
            full_path + "/models/LR_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    ...
    pickle.dump(
        model, open(full_path + "/models/pickledModels/LR_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def run_DT_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/DT_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for DT Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/DT_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/DT_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/DT_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def run_RF_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/RF_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for RF Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/RF_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/RF_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/RF_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def run_GB_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/GB_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for GB Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/GB_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/GB_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/GB_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_XGB(
    ...
    params = {
        "booster": trial.suggest_categorical("booster", param_grid["booster"]),
        ...
        "alpha": trial.suggest_loguniform(
            "alpha", param_grid["alpha"][0], param_grid["alpha"][1]
        ),
        "eta": trial.suggest_loguniform(
            "eta", param_grid["eta"][0], param_grid["eta"][1]
        ),
        "gamma": trial.suggest_loguniform(
            "gamma", param_grid["gamma"][0], param_grid["gamma"][1]
        ),
        "max_depth": trial.suggest_int(
            "max_depth", param_grid["max_depth"][0], param_grid["max_depth"][1]
        ),
        "grow_policy": trial.suggest_categorical(
            "grow_policy", param_grid["grow_policy"]
        ),
        "n_estimators": trial.suggest_int(
            "n_estimators", param_grid["n_estimators"][0], param_grid["n_estimators"][1]
        ),
        ...
        "subsample": trial.suggest_uniform(
            "subsample", param_grid["subsample"][0], param_grid["subsample"][1]
        ),
...
def run_XGB_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        sss = StratifiedShuffleSplit(
            n_splits=1, train_size=int(training_subsample), random_state=random_state
        )
        for train_index, _ in sss.split(x_train, y_train):
            x_train = x_train[train_index]
            y_train = y_train[train_index]
        print(
            "For XGB training sample reduced to " + str(x_train.shape[0]) + " instances"
        )
    # Specify algorithm for hyperparameter optimization
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/XGB_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for XGB Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        export_best_params(
            full_path + "/models/XGB_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/XGB_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/XGB_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_LGB(
    ...
    params = {
        ...
        "num_leaves": trial.suggest_int(
            "num_leaves", param_grid["num_leaves"][0], param_grid["num_leaves"][1]
        ),
        "max_depth": trial.suggest_int(
            "max_depth", param_grid["max_depth"][0], param_grid["max_depth"][1]
        ),
        ...
        "min_child_samples": trial.suggest_int(
            "min_child_samples",
            param_grid["min_child_samples"][0],
...
def run_LGB_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/LGB_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for LGB Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/LGB_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/LGB_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/LGB_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_CGB(
    ...
    params = {
        ...
        "iterations": trial.suggest_int(
            "iterations", param_grid["iterations"][0], param_grid["iterations"][1]
        ),
        "depth": trial.suggest_int(
            "depth", param_grid["depth"][0], param_grid["depth"][1]
        ),
        "l2_leaf_reg": trial.suggest_int(
            "l2_leaf_reg", param_grid["l2_leaf_reg"][0], param_grid["l2_leaf_reg"][1]
        ),
        "loss_function": trial.suggest_categorical(
            "loss_function", param_grid["loss_function"]
        ),
        "random_seed": trial.suggest_categorical(
            "random_seed", param_grid["random_seed"]
        ),
    }
...
def run_CGB_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/CGB_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for CGB Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/CGB_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/CGB_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    ...
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/CGB_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_SVM(
    ...
    params = {
        "kernel": trial.suggest_categorical("kernel", param_grid["kernel"]),
        "C": trial.suggest_loguniform("C", param_grid["C"][0], param_grid["C"][1]),
        "gamma": trial.suggest_categorical("gamma", param_grid["gamma"]),
        "degree": trial.suggest_int(
            "degree", param_grid["degree"][0], param_grid["degree"][1]
        ),
...
def run_SVM_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        ...
        for train_index, _ in sss.split(x_train, y_train):
            ...
            y_train = y_train[train_index]
        print(
            "For SVM, training sample reduced to "
            + str(x_train.shape[0])
            + " instances"
        )
    # Specify algorithm for hyperparameter optimization
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/SVM_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for SVM Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/SVM_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/SVM_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/SVM_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_ANN(
    ...
    params = {
        "activation": trial.suggest_categorical("activation", param_grid["activation"]),
        "learning_rate": trial.suggest_categorical(
            "learning_rate", param_grid["learning_rate"]
        ),
        "momentum": trial.suggest_uniform(
            "momentum", param_grid["momentum"][0], param_grid["momentum"][1]
        ),
        "solver": trial.suggest_categorical("solver", param_grid["solver"]),
    ...
    n_layers = trial.suggest_int(
        "n_layers", param_grid["n_layers"][0], param_grid["n_layers"][1]
    )
    layers = []
    for i in range(n_layers):
        layers.append(
            trial.suggest_int(
                "n_units_l{}".format(i),
                param_grid["layer_size"][0],
                param_grid["layer_size"][1],
            )
        )
...
def run_ANN_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        ...
        for train_index, _ in sss.split(x_train, y_train):
            ...
            y_train = y_train[train_index]
        print(
            "For ANN, training sample reduced to "
            + str(x_train.shape[0])
            + " instances"
        )
    # Specify algorithm for hyperparameter optimization
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/ANN_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for ANN Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/ANN_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/ANN_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/ANN_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_KNN(
    ...
    params = {
        "n_neighbors": trial.suggest_int(
            "n_neighbors", param_grid["n_neighbors"][0], param_grid["n_neighbors"][1]
        ),
...
def run_KNN_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        ...
        for train_index, _ in sss.split(x_train, y_train):
            ...
            y_train = y_train[train_index]
        print(
            "For KNN, training sample reduced to "
            + str(x_train.shape[0])
            + " instances"
        )
    # Specify algorithm for hyperparameter optimization
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/KNN_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for KNN Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/KNN_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/KNN_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/KNN_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_GP(
    ...
    params = {
        "population_size": trial.suggest_int(
            "population_size",
            param_grid["population_size"][0],
            param_grid["population_size"][1],
        ),
        "generations": trial.suggest_int(
            "generations", param_grid["generations"][0], param_grid["generations"][1]
        ),
        "tournament_size": trial.suggest_int(
            "tournament_size",
            param_grid["tournament_size"][0],
            param_grid["tournament_size"][1],
        ),
        "function_set": trial.suggest_categorical(
            "function_set", param_grid["function_set"]
        ),
        "init_method": trial.suggest_categorical(
            "init_method", param_grid["init_method"]
        ),
        "parsimony_coefficient": trial.suggest_float(
            "parsimony_coefficient",
            param_grid["parsimony_coefficient"][0],
            param_grid["parsimony_coefficient"][1],
        ),
        "feature_names": trial.suggest_categorical(
            "feature_names", param_grid["feature_names"]
        ),
...
def run_GP_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        ...
        for train_index, _ in sss.split(x_train, y_train):
            ...
            y_train = y_train[train_index]
        print(
            "For GP, training sample reduced to " + str(x_train.shape[0]) + " instances"
        )
    # Specify algorithm for hyperparameter optimization
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/GP_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for GP Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue. However failure may persist for GP specifically."
                )
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/GP_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/GP_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/GP_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_eLCS(
    ...
    params = {
        "learning_iterations": trial.suggest_categorical(
            "learning_iterations", param_grid["learning_iterations"]
        ),
...
def run_eLCS_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/eLCS_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for eLCS Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/eLCS_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/eLCS_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    ...
    pickle.dump(
        model,
        open(full_path + "/models/pickledModels/eLCS_" + str(i) + ".pickle", "wb"),
    )
    # Evaluate model
    (
        metricList,
...
def run_XCS_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/XCS_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for XCS Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/XCS_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/XCS_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/XCS_" + str(i) + ".pickle", "wb")
    )
    # Evaluate model
    (
        metricList,
...
def objective_ExSTraCS(
    ...
    params = {
        ...
        "rule_compaction": trial.suggest_categorical(
            "rule_compaction", param_grid["rule_compaction"]
        ),
    }
    return hyper_eval(
        est, x_train, y_train, random_state, hype_cv, params, scoring_metric
...
def get_FI_subset_ExSTraCS(
    full_path, i, instance_label, class_label, filter_poor_features
):
    ...
    data_name = full_path.split("/")[-1]
    if os.path.exists(
        full_path + "/multisurf/pickledForPhase4/"
    ):  # If MultiSURF was done previously:
        algorithmlabel = "multisurf"
    elif os.path.exists(
        full_path + "/mutualinformation/pickledForPhase4/"
    ...
    if eval(
        ...
        header = pd.read_csv(
            full_path + "/CVDatasets/" + data_name + "_CV_" + str(i) + "_Test.csv"
        ).columns.values.tolist()
        if instance_label != "None":
        ...
        scoreInfo = (
            full_path + "/" + algorithmlabel + "/pickledForPhase4/" + str(i) + ".pickle"
        )
...
def run_ExSTraCS_full(
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        optuna.logging.set_verbosity(optuna.logging.INFO)
        study.optimize(
            lambda trial: objective_ExSTraCS(
                trial,
                est,
                x_train,
                y_train,
        ...
        if eval(do_plot):
            try:
                fig = optuna.visualization.plot_parallel_coordinate(study)
                fig.write_image(
                    full_path + "/models/ExSTraCS_ParamOptimization_" + str(i) + ".png"
                )
            except:
                print(
                    "Warning: Optuna Optimization Visualization Generation Failed for ExSTraCS Due to Known Release Issue.  Please install Optuna 2.0.0 to avoid this issue."
                )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/ExSTraCS_bestparams" + str(i) + ".csv",
            best_trial.params,
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/ExSTraCS_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
    ...
    pickle.dump(
        model,
        open(full_path + "/models/pickledModels/ExSTraCS_" + str(i) + ".pickle", "wb"),
    )
    # Evaluate model
    (
        metricList,
...
def classEval(y_true, y_pred, probas_):
    ...
    # MultiClass cohen_kappa, matthews_corrcoef, log_loss, f1_weighted, f1_macro
    ck = cohen_kappa_score(y_true, y_pred)
    mcc = matthews_corrcoef(y_true, y_pred)
    ll = log_loss(y_true, probas_)
    ...
    if sp == 1:
        ...
        lrp = re / float(
            1 - sp
        )  # sensitivity / (1-specificity).... a.k.a. TPR/FPR... or TPR/(1-TNR)
    ...
    if sp == 0:
        ...
        lrm = (1 - re) / float(
            sp
        )  # (1-sensitivity) / specificity... a.k.a. FNR/TNR ... or (1-TPR)/TNR
    return [
        bac,
        ac,
        f1,
...
def hyperparameters(
    ...
    if eval(
        do_lcs_sweep
    ):  # Contuct hyperparameter sweep of LCS algorithms (can be computationally expensive)
        # eLCS
        param_grid_eLCS = {
