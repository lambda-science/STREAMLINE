...
def job(
    ...
    if eval(do_mutual_info):
        ...
        selected_feature_lists, meta_feature_ranks = reportAveFS(
            "Mutual Information",
            "mutualinformation",
            cv_partitions,
            top_features,
    ...
    if len(algorithms) != 0:
        if eval(filter_poor_features):
            ...
            reportInformativeFeatures(
                informativeFeatureCounts, uninformativeFeatureCounts, full_path
            )
    ...
    job_file = open(
        experiment_path
        + "/jobsCompleted/job_featureselection_"
        + dataset_name
        + ".txt",
        "w",
    )
...
def reportInformativeFeatures(
    informativeFeatureCounts, uninformativeFeatureCounts, full_path
):
    """Saves counts of informative vs uninformative features (i.e. those with feature importance scores <= 0) in an csv file."""
    counts = {
        "Informative": informativeFeatureCounts,
        "Uninformative": uninformativeFeatureCounts,
    }
    count_df = pd.DataFrame(counts)
    count_df.to_csv(
        full_path + "/feature_selection/InformativeFeatureSummary.csv",
...
def reportAveFS(
    ...
    for i in range(0,cv_partitions):
        ...
        scoreDict = rawData[
            1
        ]  # dictionary of feature importance scores (original feature order)
        score_sorted_features = rawData[
            2
    ...
    selected_feature_lists[
        algorithm
    ] = cv_keep_list  # stores feature names to keep for all algorithms and CVs
    meta_feature_ranks[
        algorithm
    ] = feature_name_ranks  # stores sorted feature importance dicitonaries for all algorithms and CVs
...
def selectFeatures(algorithms, cv_partitions, selectedFeatureLists, max_features_to_keep, metaFeatureRanks):
    ...
    if (
        ...
        for i in range(cv_partitions):
            ...
            if (
                len(unionList) > max_features_to_keep
...
def genFilteredDatasets(
    ...
    for i in range(cv_partitions):
        ...
        trainSet = pd.read_csv(
            path_to_csv + "/" + dataset_name + "_CV_" + str(i) + "_Train.csv",
            na_values="NA",
            sep=",",
        )
        ...
        testSet = pd.read_csv(
            path_to_csv + "/" + dataset_name + "_CV_" + str(i) + "_Test.csv",
            na_values="NA",
            sep=",",
        )
        ...
        if eval(overwrite_cv):
            ...
            os.rename(
                path_to_csv + "/" + dataset_name + "_CV_" + str(i) + "_Train.csv",
                path_to_csv + "/" + dataset_name + "_CVPre_" + str(i) + "_Train.csv",
            )
            os.rename(
                path_to_csv + "/" + dataset_name + "_CV_" + str(i) + "_Test.csv",
                path_to_csv + "/" + dataset_name + "_CVPre_" + str(i) + "_Test.csv",
            )
        # Write new CV files
        with open(
            path_to_csv + "/" + dataset_name + "_CV_" + str(i) + "_Train.csv",
