...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="scale_data",
        type=str,
        help="perform data scaling (required for SVM, and to use Logistic regression with non-uniform feature importance estimation)",
        default="True",
    )
    parser.add_argument(
        "--impute",
        dest="impute_data",
        type=str,
        help="perform missing value data imputation (required for most ML algorithms if missing data is present)",
        default="True",
    )
    parser.add_argument(
        "--multi-impute",
        dest="multi_impute",
        type=str,
        help="applies multivariate imputation to quantitative features, otherwise uses median imputation",
        default="True",
    )
    parser.add_argument(
        "--over-cv",
    ...
    if not os.path.exists(options.output_path):
        raise Exception(
            "Output path must exist (from phase 1) before phase 2 can begin"
        )
    ...
    if not options.do_check:  # Run job file
        ...
        for dataset in datasets:
            for cv in range(cv_partitions):
                phase2Jobs.append(
                    "job_preprocessing_" + dataset + "_" + str(cv) + ".txt"
                )
        ...
        for filename in glob.glob(
            ...
            + "/"
            + options.experiment_name
            + "/jobsCompleted/job_preprocessing*"
        ):
