...
def runExplore(
    ...
    topFeatures = (
        20  # only used with jupyter notebook reporting of univariate analyses.
    )
    ...
    if not match_label == "None":
        ...
        if not match_label in dfHeader:
            ...
            partition_method = "S"
            print(
                "Warning: Specified 'Match label' could not be found in dataset. Analysis moving forward assuming there is no 'match label' column using stratified (S) CV partitioning."
            )
    ...
    if instance_label == "None" and match_label == "None":
        ...
        x_data = data.drop(
            [class_label, instance_label], axis=1
        ...
        x_data = data.drop(
            [class_label, instance_label, match_label], axis=1
        )  # exclude class column
    # Automatically identify categorical vs. quantitative features/variables
    if eval(jupyterRun):
        print("Identifying Feature Types...")
    categorical_variables = idFeatureTypes(
        x_data,
        categorical_feature_headers,
        categorical_cutoff,
        experiment_path,
    ...
    sorted_p_list = univariateAnalysis(
        data,
        experiment_path,
        dataset_name,
        class_label,
    ...
    train_dfs, test_dfs = cv_partitioner(
        data, cv_partitions, partition_method, class_label, match_label, random_state
    )
    ...
    job_file = open(
        experiment_path + "/jobsCompleted/job_exploratory_" + dataset_name + ".txt", "w"
    )
...
def idFeatureTypes(
    ...
    if (
        len(categorical_feature_headers) == 0
    ):  # Runs unless user has specified a predifined list of variables to treat as categorical
        categorical_variables = []
        for each in x_data:
            if x_data[
                each
            ].nunique() <= categorical_cutoff or not pd.api.types.is_numeric_dtype(
                x_data[each]
            ):
    ...
    outfile = open(
        ...
        + "/"
        + dataset_name
        + "/exploratory/categorical_variables.pickle",
        "wb",
    )
...
def describeData(data, experiment_path, dataset_name):
    ...
    data.describe().to_csv(
        experiment_path + "/" + dataset_name + "/exploratory/" + "DescribeDataset.csv"
    )
    data.dtypes.to_csv(
        experiment_path + "/" + dataset_name + "/exploratory/" + "DtypesDataset.csv",
        header=["DataType"],
        index_label="Variable",
    )
    data.nunique().to_csv(
        experiment_path + "/" + dataset_name + "/exploratory/" + "NumUniqueDataset.csv",
        header=["Count"],
        index_label="Variable",
    )
...
def missingnessCounts(data, experiment_path, dataset_name, jupyterRun):
    ...
    missing_count.to_csv(
        experiment_path + "/" + dataset_name + "/exploratory/" + "DataMissingness.csv",
        header=["Count"],
        index_label="Variable",
    )
...
def countsSummary(
    ...
    summary = [
        ...
        ["features", fCount],
        ["categorical_features", len(categorical_variables)],
        ["quantitative_features", fCount - len(categorical_variables)],
        ["missing_values", totalMissing],
        ["missing_percent", round(percentMissing, 5)],
    ]
    dfSummary = pd.DataFrame(summary, columns=["Variable", "Count"])
    dfSummary.to_csv(
        experiment_path + "/" + dataset_name + "/exploratory/" + "DataCounts.csv",
        index=None,
    )
    # Calculate, print, and export class counts
    class_counts = data[class_label].value_counts()
    class_counts.to_csv(
        experiment_path + "/" + dataset_name + "/exploratory/" + "ClassCounts.csv",
        header=["Count"],
        index_label="Class",
    )
    print("Class Counts: ----------------")
...
def reportHeaders(x_data, experiment_path, dataset_name):
    ...
    with open(
        experiment_path + "/" + dataset_name + "/exploratory/OriginalFeatureNames.csv",
        mode="w",
        newline="",
    ) as file:
        writer = csv.writer(
...
def univariateAnalysis(
    ...
    try:  # Try loop added to deal with versions specific change to using mannwhitneyu in scipy and avoid STREAMLINE crash in those circumstances.
        ...
        if not os.path.exists(
            experiment_path + "/" + dataset_name + "/exploratory/univariate_analyses"
        ):
            os.mkdir(
                experiment_path
                + "/"
        ...
        for column in data:
            if column != class_label and column != instance_label:
                p_value_dict[column] = test_selector(
                    column, class_label, data, categorical_variables
                )
...
def test_selector(featureName, class_label, data, categorical_variables):
    ...
    # Feature is continuous and Outcome is discrete/categorical/binary
        ...
        try:  # works in scipy 1.5.0
            c, p = scs.mannwhitneyu(
                x=data[featureName].loc[data[class_label] == 0],
    ...
    for i in sorted_p_list:  # each feature in sorted p-value dictionary
        if i[1] == "None":
            ...
            for j in data:  # each feature
                if (
                    j == i[0] and i[1] <= sig_cutoff
                ):  # ONLY EXPORTS SIGNIFICANT FEATURES
                    graph_selector(
                        j,
                        class_label,
                        data,
...
def graph_selector(
    featureName, class_label, data, categorical_variables, experiment_path, dataset_name
):
    ...
    if (
        featureName in categorical_variables
    ):  # Feature and Outcome are discrete/categorical/binary
...
def cv_partitioner(
    data, cv_partitions, partition_method, class_label, match_label, randomSeed
):
    ...
    if not match_label == "None":
        matchIndex = data.columns.get_loc(
            match_label
    ...
    # Stratified Partitioning Method------------------------------------------------------------
        ...
        ]  # create list of empty lists (one for each class)
        for row in datasetList:
    ...
    # Matched partitioning method ---------------------------------------------------------------
        ...
        ]  # create list of empty lists (one for each match group)
        for row in datasetList:
...
def saveRuntime(experiment_path, dataset_name, job_start_time):
    ...
    runtime_file = open(
        experiment_path + "/" + dataset_name + "/runtime/runtime_exploratory.txt", "w"
    )
