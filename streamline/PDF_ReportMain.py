...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="training",
        type=str,
        help="Indicate True or False for whether to generate pdf summary for pipeline training or followup application analysis to new dataset",
        default="True",
    )
    parser.add_argument(
        "--rep-path",
    ...
    if not options.training == "True":
        if options.rep_data_path == "None" or options.dataset_for_rep == "None":
            raise Exception(
                "Replication and Dataset paths must be specified as arguments to generate PDF summary on new data analysis!"
            )
    ...
    if not options.do_check:  # Run job submission
        ...
        if options.training == "True":  # Make pdf summary for training analysis
            ...
            for filename in glob.glob(
                ...
                if (
                    filename.split("/")[-1]
                    == "job_data_pdf_apply_" + str(train_name) + ".txt"
                ):
