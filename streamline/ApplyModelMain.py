...
def main(argv):
    ...
    parser.add_argument(
        ...
        dest="rep_data_path",
        type=str,
        help="path to directory containing replication or hold-out testing datasets (must have at least all features with same labels as in original training dataset)",
    )
    parser.add_argument(
        "--dataset",
        dest="dataset_for_rep",
        type=str,
        help="path to target original training dataset",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="match_label",
        type=str,
        help="applies if original training data included column with matched instance ids",
        default="None",
    )
    ...
    data_name = options.dataset_for_rep.split("/")[-1].split(".")[
        0
    ...
    if not os.path.exists(options.output_path):
        raise Exception(
            "Output path must exist (from phase 1-5) before model application can begin"
        )
    if not os.path.exists(options.output_path + "/" + options.experiment_name):
        raise Exception(
            "Experiment must exist (from phase 1-5) before model application can begin"
        )

    full_path = (
        options.output_path + "/" + options.experiment_name + "/" + data_name
    ...
    if not options.do_check:  # Run job submission
        ...
        if not eval(jupyterRun):
            if not os.path.exists(
                options.output_path + "/" + options.experiment_name + "/jobs"
            ):
                os.mkdir(options.output_path + "/" + options.experiment_name + "/jobs")
            if not os.path.exists(
                options.output_path + "/" + options.experiment_name + "/logs"
            ):
                ...
                if apply_name not in unique_datanames:
                    ...
                    if eval(options.run_parallel):
                        submitClusterJob(
                            ...
                            impute_data,
                            primary_metric,
                            options.dataset_for_rep,
                            options.match_label,
                            options.plot_ROC,
                            options.plot_PRC,
                            options.plot_metric_boxplots,
        ...
        if file_count == 0:  # Check that there was at least 1 dataset
            raise Exception(
                "There must be at least one .txt or .csv dataset in rep_data_path directory"
            )
