...
def job(
    ...
    if not os.path.exists(
        full_path + "/applymodel/" + apply_name + "/" + "exploratory"
    ):
        os.mkdir(full_path + "/applymodel/" + apply_name + "/" + "exploratory")
    if not os.path.exists(
        full_path + "/applymodel/" + apply_name + "/" + "model_evaluation"
    ):
    ...
    ExploratoryAnalysisJob.describeData(
        repData, full_path, "applymodel/" + apply_name
    )  # Arguments changed to send to correct locations describeData(data,experiment_path,dataset_name)
    totalMissing = ExploratoryAnalysisJob.missingnessCounts(
        repData, full_path, "applymodel/" + apply_name, jupyterRun
    )
    ExploratoryAnalysisJob.countsSummary(
        repData,
        class_label,
        full_path,
        "applymodel/" + apply_name,
        instance_label,
        match_label,
    ...
    if eval(export_feature_correlations):
        ExploratoryAnalysisJob.featureCorrelationPlot(
            x_repData, full_path, "applymodel/" + apply_name, jupyterRun
        )
    ...
    for cvCount in range(0, cv_partitions):
        ...
        masterList.append(
            evalDict
        )  # update master list with evalDict for this CV model
    ...
    StatsJob.doPlotROC(
        result_table, colors, full_path + "/applymodel/" + apply_name, jupyterRun
    )
    doPlotPRC(
        result_table,
        colors,
    ...
    if len(algorithms) > 1:
        kruskal_summary = StatsJob.kruskalWallis(
            full_path + "/applymodel/" + apply_name,
            metrics,
            algorithms,
            metric_dict,
    ...
    job_file = open(
        experiment_path + "/jobsCompleted/job_apply_" + apply_name + ".txt", "w"
    )
...
def imputeRepData(
    ...
    imputeCatInfo = (
        full_path + "/scale_impute/categorical_imputer_cv" + str(cvCount) + ".pickle"
    ...
    for c in cvRepData.columns:
        if (
            c in mode_dict
        ):  # was the given feature identified as and treated as categorical during training?
            cvRepData[c].fillna(mode_dict[c], inplace=True)

    imputeOridinalInfo = (
        full_path + "/scale_impute/ordinal_imputer_cv" + str(cvCount) + ".pickle"
    )  # Corresponding pickle file name with scalingInfo
    if eval(multi_impute):  # multiple imputation of quantitative features
        ...
        if instance_label == None or instance_label == "None":
            ...
            inst_rep = cvRepData[
                instance_label
            ].values  # pull out instance labels in case they include text
...
def evalModel(full_path, algAbrev, cvRepDataX, cvRepDataY, cvCount):
    ...
    prec, recall, thresholds = (
        prec[::-1],
        recall[::-1],
        thresholds[::-1],
    )  # reversed list orders
...
def doPlotPRC(
    ...
    plt.savefig(
        full_path + "/applymodel/" + apply_name + "/model_evaluation/Summary_PRC.png",
        bbox_inches="tight",
    )
    if eval(jupyterRun):
        plt.show()
