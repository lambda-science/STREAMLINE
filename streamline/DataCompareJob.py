...
def job(experiment_path, sig_cutoff, jupyterRun):
    ...
    data = pd.read_csv(
        dataset_directory_paths[0] + "/model_evaluation/Summary_performance_mean.csv",
        sep=",",
    )
    ...
    global_data = bestKruscallWallis(
        experiment_path,
        datasets,
        algorithms,
        metrics,
...
def wilcoxonRank(
    ...
    for algorithm in algorithms:
        ...
        df.columns = label
        df.to_csv(
            experiment_path + "/DatasetComparisons/WilcoxonRank_" + algorithm + ".csv",
            index=False,
        )
...
def mannWhitneyU(
    ...
    for algorithm in algorithms:
        ...
        df.columns = label
        df.to_csv(
            experiment_path + "/DatasetComparisons/MannWhitney_" + algorithm + ".csv",
            index=False,
        )


def bestKruscallWallis(
    experiment_path,
    datasets,
    algorithms,
    metrics,
...
def bestMannWhitneyU(
    ...
    df.columns = label
    df.to_csv(
        experiment_path + "/DatasetComparisons/BestCompare_MannWhitney.csv", index=False
    )


def bestWilcoxonRank(
    experiment_path,
    datasets,
    algorithms,
    metrics,
    ...
    df.columns = label
    df.to_csv(
        experiment_path + "/DatasetComparisons/BestCompare_WilcoxonRank.csv",
        index=False,
    )


def dataCompareBPAll(
    experiment_path, metrics, dataset_directory_paths, algorithms, colors, jupyterRun
):
    ...
    for metric in metrics:  # One boxplot generated for each available metric
        ...
        for each in dataset_directory_paths:  # each target dataset
            ...
            data = pd.read_csv(
                each + "/model_evaluation/Summary_performance_mean.csv",
                sep=",",
                index_col=0,
            )
        ...
        for i in range(len(algorithms)):
            plt.plot(
                np.arange(len(dataset_directory_paths)) + 1,
                alg_values_dict[algorithms[i]],
                color=colors[algorithms[i]],
                label=algorithms[i],
            )
        # Specify plot labels
        ...
        plt.savefig(
            experiment_path
            + "/DatasetComparisons/dataCompBoxplots/DataCompareAllModels_"
            + metric
            + ".png",
            bbox_inches="tight",
        )
...
def dataCompareBP(
    ...
    for algorithm in algorithms:
        for metric in metricList:
            ...
            plt.savefig(
                experiment_path
                + "/DatasetComparisons/dataCompBoxplots/DataCompare_"
                + name_to_abbrev[algorithm]
                + "_"
                + metric
                + ".png",
