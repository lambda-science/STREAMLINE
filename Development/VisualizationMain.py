...
def main(argv):
    ...
    parser.add_argument(
        "--output-path", dest="output_path", type=str, help="path to output directory"
    )
    parser.add_argument(
        "--experiment-name",
    ...
    if not os.path.exists(output_path):
        raise Exception(
            "Output path must exist (from phase 1) before phase 7 can begin"
        )
    ...
    metadata = pd.read_csv(
        output_path + "/" + experiment_name + "/" + "metadata.csv"
    ).values
